...
class ActorCriticNetwork(object):
	def __init__(self, act_size, opt):
		...
		self.optimizer = opt
		self.fc1, self.fc1_b = self.fc_layer([1024, 512])
		self.fc2, self.fc2_b = self.fc_layer([512, 256])
		self.fc3, self.fc3_b = self.fc_layer([128, 64])
		self.fc4, self.fc4_b = self.fc_layer([64, act_size])
		self.fc5, self.fc5_b = self.fc_layer([64, 1])
		self.state = tf.placeholder(tf.float32, [None, 512, 2])
		h_fc1 = tf.nn.relu(tf.matmul(tf.reshape(self.state, [-1, 1024]), self.fc1) + self.fc1_b)
	...
	def set_up_loss(self, entropy_var):
		...
		log_policy = tf.log(tf.clip_by+value(self.policy_out, 1e-15, 1.0))
